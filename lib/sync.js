...
exports.init = function() {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (process.env.CUS_MONGODB_CONN_URL) {
            // Read in environment variables required to initialise sync
            const MONGO_URL = env('CUS_MONGODB_CONN_URL').asString();
            const REDIS_HOST = env('CUS_REDIS_HOST').required().asString();
            const REDIS_PORT = env('CUS_REDIS_PORT').required().asPositiveInt();
            ...
            if (MONGO_URL) {
                mbaasApi.sync.connect(MONGO_URL, {}, `redis://${REDIS_HOST}:${REDIS_PORT}`, connectCallback);
            } else {
